    ...
    def state(self) -> CircuitBreakerState:
        ...
        with self._lock:
            ...
            if (self._state == CircuitBreakerState.OPEN and 
                ...
                self._success_count = 0
                log_databricks_event("CIRCUIT_BREAKER", "HALF_OPEN", f"{self.name} entering half-open state")
        ...
        with self._lock:
            if self._state == CircuitBreakerState.HALF_OPEN:
                ...
                if self._success_count >= self.config.success_threshold:
                    self._state = CircuitBreakerState.CLOSED
                    self._failure_count = 0
                    log_databricks_event("CIRCUIT_BREAKER", "CLOSED", f"{self.name} circuit closed after recovery")
        ...
        with self._lock:
            ...
            if self._state == CircuitBreakerState.CLOSED:
                if self._failure_count >= self.config.failure_threshold:
                    ...
                    log_databricks_event("CIRCUIT_BREAKER", "OPEN", f"{self.name} circuit opened due to failures")
            elif self._state == CircuitBreakerState.HALF_OPEN:
                self._state = CircuitBreakerState.OPEN
                log_databricks_event("CIRCUIT_BREAKER", "OPEN", f"{self.name} circuit reopened during half-open test")
...
class ErrorHandler:
    ...
    def classify_error(self, error: Exception) -> ErrorType:
        ...
        elif any(keyword in error_str for keyword in ['authentication', 'unauthorized', 'forbidden', 'token']):
            return ErrorType.AUTHENTICATION
        elif any(keyword in error_str for keyword in ['rate limit', 'too many requests', '429']):
            return ErrorType.RATE_LIMIT
        elif any(keyword in error_str for keyword in ['databricks', 'api error', 'rest api']):
    ...
    def with_retry(self, operation_name: str, config: Optional[RetryConfig] = None, 
        ...
        def decorator(func: Callable) -> Callable:
            ...
            def wrapper(*args, **kwargs):
                ...
                circuit = self.get_circuit_breaker(operation_name) if circuit_breaker else None
                
                # Check circuit breaker
                if circuit and not circuit.can_execute():
                    error_msg = f"Circuit breaker {operation_name} is OPEN - blocking execution"
                    log_databricks_event("ERROR_HANDLER", "BLOCKED", error_msg, "WARNING")
...
def with_retry(operation_name: str, max_attempts: int = 3, base_delay: float = 1.0, 
