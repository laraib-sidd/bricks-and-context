    ...
    def get(self, category: str, identifier: str) -> Optional[Any]:
        ...
        with self._lock:
            ...
            if entry.is_expired:
                del self._cache[key]
                self._misses += 1
                log_databricks_event("CACHE", "EXPIRED", f"Cache expired for {category}:{identifier}")
                return None
            
            self._hits += 1
            log_databricks_event("CACHE", "HIT", f"Cache hit for {category}:{identifier} (age: {entry.age_seconds}s)")
            return entry.value
    
    def set(self, category: str, identifier: str, value: Any, ttl_seconds: Optional[int] = None) -> bool:
        """
        ...
        with self._lock:
            ...
            if len(self._cache) >= self._max_entries:
                self._evict_oldest(max(1, self._max_entries // 10))  # Evict 10% when full
            ...
            log_databricks_event("CACHE", "SET", f"Cached {category}:{identifier} (TTL: {ttl_seconds}s)")
            return True
        ...
        with self._lock:
            if identifier:
                ...
                if key in self._cache:
                    del self._cache[key]
                    log_databricks_event("CACHE", "INVALIDATE", f"Invalidated {category}:{identifier}")
                    return 1
                return 0
            else:
                ...
                if keys_to_remove:
                    log_databricks_event("CACHE", "INVALIDATE", f"Invalidated {len(keys_to_remove)} {category} entries")
...
def cache_table_schema(schema_name: str, table_name: str, schema_info: dict, ttl_seconds: int = 900) -> bool:
    """Cache table schema information."""
    cache = get_cache_manager()
    return cache.set('table', f'schema_{schema_name}_{table_name}', schema_info, ttl_seconds)
